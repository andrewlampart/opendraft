...
def export_docx(
    ...
    try:
        ...
        reference_doc = (
            Path(__file__).parent.parent / "examples" / "custom-reference.docx"
        )

        if not reference_doc.exists():
            logger.warning(f"Reference document not found: {reference_doc}")
            logger.warning(
                "Continuing without reference document (may lose some formatting)"
            )
            reference_doc = None
        ...
        if not insert_academic_structure(
            ...
            logger.warning(
                "Post-processing failed - DOCX created but may lack page structure"
            )
            logger.warning(
                "DOCX will have inline title block instead of standalone pages"
            )
...
def show_available_engines() -> None:
    ...
    if not engines:
        ...
        logger.info(
            "  - LibreOffice: sudo apt install libreoffice-writer libreoffice-core-nogui"
        )
        logger.info(
...
Engine Options:
  ...
  weasyprint   - Use WeasyPrint (fallback, known font rendering issues)
    ...
    if args.pdf:
        pdf_success, _pdf_err = export_pdf(input_file, Path(args.pdf), engine=args.engine)
