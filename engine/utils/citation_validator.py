...
class CitationValidator:
    ...
    def validate_doi(self, doi: str) -> Optional[bool]:
        ...
        try:
            response = requests.get(
                ...
                headers={
                    "User-Agent": "OpenDraft/1.3 (https://github.com/federicodeponte/opendraft)"
                },
            )
    ...
    def check_author_sanity(self, authors: List[str]) -> List[str]:
        ...
        for author in authors:
            ...
            if re.search(
                r"\.(com|org|gov|edu|net|io|ai|co\.uk)(:443)?$", author, re.IGNORECASE
            ):
    ...
    def validate_database(
        ...
        stats = {
            ...
            "invalid_authors": sum(
                1 for i in all_issues if i.issue_type == "invalid_author"
            ),
            "invalid_urls": sum(1 for i in all_issues if i.issue_type == "invalid_url"),
            "invalid_metadata": sum(
                1 for i in all_issues if i.issue_type == "invalid_metadata"
            ),
        }
    ...
    def generate_report(self, issues: List[ValidationIssue], draft_name: str) -> str:
        ...
        report.append(f"\n{'='*80}")
        report.append(
            f"TOTAL ISSUES: {len(issues)} ({len(critical)} critical, {len(warnings)} warnings)"
        )
        report.append(f"{'='*80}\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Validate citations for academic integrity"
    )
    parser.add_argument(
        "database", nargs="?", help="Path to citation_database.json (or validate all)"
    )
    parser.add_argument(
        "--all", action="store_true", help="Validate all 3 draft databases"
    )
    ...
    if args.all or not args.database:
        ...
        databases = [
            (
                base_dir / "tests/outputs/opensource_draft/citation_database.json",
                "Open Source Draft",
            ),
            (
                base_dir / "tests/outputs/ai_pricing_draft/citation_database.json",
                "AI Pricing Draft",
            ),
            (
                base_dir / "tests/outputs/co2_draft_german/citation_database.json",
                "CO2 German Draft",
            ),
        ]
    else:
