...
class BackpressureManager:
    ...
    def signal_429(self, api_type: APIType, proxy_id: Optional[str] = None) -> None:
        ...
        logger.warning(
            f"429 signaled for {api_type.value} (count: {current_count + 1})"
        )
    ...
    def get_recommended_delay(self) -> float:
        ...
        self._recalculate_pressure()
        return self._get(
            "global:recommended_delay", PRESSURE_CONFIG["min_delay_seconds"]
        )
    ...
    def get_best_gemini_key(
        ...
        logger.debug(
            f"Selected {best_type.value} (429s: {count}) from {len(keys)} keys"
        )
