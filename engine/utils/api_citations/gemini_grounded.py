...
class GeminiGroundedClient(BaseAPIClient):
    ...
    def __init__(
        ...
        Args:
            ...
            timeout: Request timeout in seconds (override with GEMINI_GROUNDED_TIMEOUT)
        ...
        raw = os.getenv("GEMINI_GROUNDED_TIMEOUT", "").strip()
        if raw.isdigit():
            timeout = int(raw)
        ...
        try:
            ...
            try:
                ...
                if fallback.dataforseo.enabled:
                    logger.info(
                        f"Gemini error - trying DataForSEO fallback for: {query[:50]}..."
                    )
        ...
        try:
            ...
            for attempt in range(2):
                try:
                    ...
                    if attempt == 0:
                        logger.info(
                            "Gemini Grounded read timeout (%ss); retrying once",
                            self.timeout,
                        )
                        time.sleep(2)
                        continue
            ...
            if not response.ok:
                ...
                if response.status_code == 429:
                    try:
                        ...
                        available_fallbacks = [
                            k for k in self._fallback_keys.values() if k
                        ]
...
Provide the source title, URL, and a brief snippet explaining relevance."""
    ...
    def _fetch_pubmed_metadata(
        self, pmid: str, original_url: str
    ...
    def _fetch_pmc_metadata(
        self, pmcid: str, original_url: str
    ) -> Optional[Dict[str, Any]]:
