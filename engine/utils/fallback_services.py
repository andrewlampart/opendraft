...
def search_web_dataforseo(
    ...
    with DATAFORSEO_LOCK:
        try:
            ...
            logger.info(
                f"[DataForSEO] Found {len(results)} results for query: {query[:50]}..."
            )
            ...
            return {
                "success": False,
                "error": "DataForSEO request timed out",
                "results": [],
            }
        ...
        async with AsyncWebCrawler(config=browser_config) as crawler:
            ...
            if not result.success:
                ...
                if "ERR_NAME_NOT_RESOLVED" in err_str:
                    ...
                    error_msg = (
                        f"ERROR: Connection refused - website may be down: {url}"
                    )
                elif (
                    "ERR_CONNECTION_TIMED_OUT" in err_str
...
def get_url_context_simple(url: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"[SimpleURLContext] Extracted {len(content)} chars from: {url[:50]}..."
        )
...
def get_tool_context_with_fallback(
    ...
    if "web-search" in tools:
        ...
        if any(
            kw in prompt_lower for kw in ["stock", "price", "share", "ticker", "market"]
        ):
