...
def run_agent(
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            if not response.candidates:
                raise ValueError(
                    f"Agent '{name}': No candidates in response (likely blocked)"
                )
            ...
            if not output:
                raise ValueError(
                    f"Agent '{name}': Unable to extract text from response"
                )
            ...
            if validators and not skip_validation:
                ...
                for i, validator in enumerate(validators):
                    logger.debug(
                        f"Agent '{name}': Running validator {i+1}/{len(validators)}"
                    )
                    ...
                    if not result.is_valid:
                        ...
                        if attempt < max_retries - 1:
                            ...
                            logger.debug(
                                f"Agent '{name}': Backing off for {backoff_seconds}s"
                            )
                            time.sleep(backoff_seconds)
                ...
                if validation_passed:
                    logger.info(
                        f"Agent '{name}': All {len(validators)} validators passed"
                    )
                    break
            else:
            ...
            if attempt < max_retries - 1 and _is_transient_error(e):
                backoff_seconds = 2**attempt
                logger.debug(
                    f"Agent '{name}': Transient error, retrying after {backoff_seconds}s"
                )
                time.sleep(backoff_seconds)
                continue
            else:
    ...
    if save_to:
        try:
            ...
            logger.info(
                f"Agent '{name}': Saved output to {save_to} ({file_size} bytes)"
            )

        except Exception as e:
            logger.error(
                f"Agent '{name}': Failed to save output to {save_to}: {str(e)}"
            )
            raise
    ...
    try:
        ...
        if save_path.exists() and save_path.stat().st_size > 0:
            ...
            if len(content.strip()) > 50:  # Non-trivial content
                logger.info(
                    f"Agent '{agent_name}': Found partial output in {save_path}"
                )
                return content
        ...
        if most_recent.stat().st_size > 0:
            ...
            if len(content.strip()) > 50:
                logger.info(
                    f"Agent '{agent_name}': Found partial output in {most_recent}"
                )
                return content
...
def research_citations_via_api(
    ...
    if use_deep_research:
        ...
        planner = DeepResearchPlanner(
            gemini_model=model, min_sources=min_sources_deep, verbose=verbose
        )
        ...
        try:
            ...
            if verbose:
                ...
                safe_print(
                    f"   Estimated Coverage: {planner.estimate_coverage(research_topics)} sources"
                )
                safe_print(f"\n📝 Research Strategy:")
            ...
            logger.warning(
                f"Deep research planning timed out, falling back to standard mode: {e}"
            )
            if verbose:
            ...
            logger.warning(
                f"Deep research planning failed, falling back to standard mode: {e}"
            )
            if verbose:
            ...
            with ThreadPoolExecutor(max_workers=1) as executor:
                ...
                try:
                    ...
                    return (
                        ...
                        research_topic,
                        [],
                        f"Timeout after {per_topic_timeout_seconds}s",
                    )
        except Exception as e:
    ...
    if PARALLEL_WORKERS > 1:
        ...
        for batch_start in range(0, total_topics, BATCH_SIZE):
            ...
            batch = list(
                enumerate(research_topics[batch_start:batch_end], batch_start + 1)
            )
            ...
            with ThreadPoolExecutor(max_workers=PARALLEL_WORKERS) as executor:
                futures = {
                    executor.submit(_research_single_topic, item): item
                    for item in batch
                }
                ...
                for future in as_completed(futures):
                    ...
                    if error:
                        ...
                        logger.error(
                            f"Citation research failed for '{research_topic}': {error}"
                        )
                    elif citations_list:
                        ...
                        if verbose:
                            ...
                            sources_str = ", ".join(
                                [c.api_source or "Unknown" for c in citations_list]
                            )
                            first_citation = citations_list[0]
                            authors_str = (
                                first_citation.authors[0]
                                if first_citation.authors
                                else "Unknown"
                            )
                            count_str = (
                                f" (+{len(citations_list)-1} more)"
                                if len(citations_list) > 1
                                else ""
                            )
                            safe_print(
                                f"✅ {authors_str} et al. ({first_citation.year}) [{sources_str}]{count_str}"
                            )
        ...
        for idx, research_topic in enumerate(research_topics, 1):
            ...
            if verbose:
                safe_print(
                    f"[{idx}/{len(research_topics)}] 🔎 {research_topic[:65]}{'...' if len(research_topic) > 65 else ''}"
                )
            ...
            try:
                ...
                with ThreadPoolExecutor(max_workers=1) as executor:
                    future = executor.submit(
                        researcher.research_citation, research_topic
                    )
                    try:
                        citations_list = future.result(
                        ...
                        if verbose:
                            safe_print(
                                f"    ⏱️  Timeout after {per_topic_timeout_seconds}s"
                            )
                        logger.warning(
                            f"Citation research timed out for '{research_topic}' after {per_topic_timeout_seconds}s"
                        )
                        continue
                ...
                if citations_list:
                    ...
                    try:
                        debug_log_path = os.getenv(
                            "DEBUG_LOG_PATH", "/tmp/opendraft/debug.log"
                        )
                ...
                logger.error(
                    f"Citation research failed for '{research_topic}': {str(e)}"
                )

    # Calculate success metrics
    citation_count = len(citations)
    success_rate = (
        (citation_count / len(research_topics) * 100) if research_topics else 0
    )
    ...
    if citation_count >= excellent_threshold:
        ...
        logger.info(
            f"Quality gate: EXCELLENT - {citation_count}/{target_minimum} citations"
        )
        ...
        if verbose:
            ...
            safe_print(
                f"    ⚠️  WARNING: Citation count is below recommended standards."
            )
            safe_print(
    ...
    markdown_lines.extend(["", "---", "", "## Citations Found", ""])
    ...
    logger.info(
        f"Scout completed: {citation_count} citations, {success_rate:.1f}% success rate"
    )
