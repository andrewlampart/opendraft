    ...
    for i, para in enumerate(doc.paragraphs):
        ...
        if in_abstract:
            ...
            if (
                style == "Heading 1"
                and text
                and (text[0].isdigit() or text.startswith("1"))
            ):
...
def _insert_institution_block(
    doc: Document, title_idx: int, options: Dict, verbose: bool
):
    ...
    if options.get("institution"):
        items.append(
            ("institution", options["institution"].upper(), False, 14)
        )  # uppercase
...
def _insert_metadata_after_date(
    doc: Document, date_idx: int, options: Dict, verbose: bool
):
    ...
    if options.get("course"):
        ...
        insert_after = new_para
        new_para = _insert_para_after(
            insert_after, options["course"], size=12, bold=True
        )
        insert_after = new_para
        additions += 3
    ...
    if options.get("system_credit"):
        ...
        insert_after = new_para
        new_para = _insert_para_after(
            insert_after, options["system_credit"], size=9, italic=True
        )
        insert_after = new_para
        additions += 2
